...
def main():
    ...
    parser.add_argument('--lr', default=configs.learning_rate, type=float,
                        help='learning rate for optimizer, default 1e-3')
    parser.add_argument('--model', default="mtrcnn", help="model to train, default mtrcnn")
    parser.add_argument('--optimizer', default="sgd", help="optimizer for training, default sgd")
    parser.add_argument('--momentum', default=configs.momentum, type=float, help='momentum for sgd, default 0.9')
    parser.add_argument('--weight_decay', default=configs.weigth_decay, type=float,
    ...
    parser.add_argument('--sgd_adjust', default=configs.sgd_adjust, type=int,
                        help='sgd method adjust lr 0 for step 1 for min, default 1')
    parser.add_argument('--sgd_step', default=configs.sgd_step, type=int,
                        help='number of steps to adjust lr for sgd, default 5')
    parser.add_argument('--sgd_gamma', default=configs.sgd_gamma, type=float,
    ...
    train_tool_list, test_tool_list, val_tool_list, train_phase_list, test_phase_list, val_phase_list = train_label[:,
                                                                                                        :-1], test_label[
                                                                                                              :, :-1], \
    ...
    train_dataloader = DataLoader(
        dataset=train_dataset, batch_size=opts.batch_size, num_workers=0, pin_memory=False, shuffle=True,
        drop_last=True)
    test_dataloader = DataLoader(
        dataset=test_dataset, batch_size=opts.batch_size, num_workers=0, pin_memory=False, shuffle=False,
        drop_last=True)
    ...
    # 模型
    endotnet = EndotNet()
    model = endotnet.cuda() if use_gpu else endotnet
    ...
    if opts.optimizer == 'sgd':
        optimizer = torch.optim.SGD([
            {"params": model.module.feature_extraction.parameters()},
            {"params": model.module.dense1.parameters(),"lr":opts.lr},
            {"params": model.module.dense2.parameters(),"lr":opts.lr},
            {"params": model.module.tool_proj.parameters(),"lr":opts.lr},
            {"params": model.module.phase_proj.parameters(),"lr":opts.lr}
        ], lr=opts.lr / 10, momentum=opts.momentum,
